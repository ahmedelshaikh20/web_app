...
export default function TableList() {
  ...
  let unmounted = false;
    ...
    axios.get('http://localhost:3000/api/user/coordinator/viewCourseSlots', {
    ...
    .then(function (response) {
      if(!unmounted){
       ...
       for(let element2 of element['Slots']){
            
            facc.push([element['Course Name'],element2['Day'], element2['Slot'], element2['Location'],element2['Available']]);
       ...
       if(elements.length == 0){message.info("You Do Not Coordinate Any Course!")}
       setTest(elements)
       setFaculty(facc);
      }
    })
...
/>
const enterLocation=  <CustomInput
...
const [typez,setType] = useState("")
  ...
  function handleModify(event) {
    ...
    if(typez=="Add Slot")
    {
      ...
      axios.post('http://localhost:3000/api/user/coordinator/addCourseSlot', {
      ...
      .catch(function (error) {
        ...
        var slot = "" + event.target.slot.value;
        var location = "" + event.target.location.value;
        axios.post('http://localhost:3000/api/user/coordinator/deleteCourseSlot', {
      ...
      var location = "" + event.target.location.value;
      axios.post('http://localhost:3000/api/user/coordinator/updateCourseSlot', {
        course:courseID,
        day:day,
        slot:slot,
        location:location,
      ...
      <GridItem xs={12} sm={12} md={12} >
        <Card>
          <CardHeader color="primary">
            <h4 className={classes.cardTitleWhite}>Modify Course Slots</h4>
        ...
        <InputLabel  htmlFor="age-native-required">Add/Update/Delete</InputLabel>
        <Select
          native
        ...
        >
          ...
          <option>Add Slot</option>
          <option>Delete Slot</option>
          <option>Update Slot</option>
       ...
       <AppBar position="relative" color="primary" style={{ top: 30 }}> 
          ...
          for(let element of faculty){
            ...
            var temp5;
              if(!element[0].localeCompare(value)){
                    temp.push([element[1], element[2], element[3], element[4]]);
                    temp2 = element[0];
                    temp3 = element[1];
                    temp4 = element[2];
                    temp5 = element[4]
        ...
        tableHead={["Day", "Slot", "Location", "Available"]}
        tableData= {temp}
